...
primitives, no all-in-one verifier (GR6). Eight refusal reasons, each a code: a runbook that
cannot tell `wrong_audience` from `unknown_key` sends you to the wrong layer.
...
forged result changes treatment); the staff console; three bearer credentials (partner token,
Cloudflare API token, `EDGE_HMAC_SECRET`).
...
are deleted on arrival. Cross-audience replay — a real, correctly
...
mailbox is indistinguishable from a legitimate one. The service token is a bearer credential with
no proof of possession. An authorised user exporting everything on their last
...
- **Drill 4, staff console down** — five-step triage, steps 4 and 5 never reached: **3s to
  diagnose, 14s to recover.** Small because the fault was; it establishes ordering, not duration.
...
status was the tell: that is DNS failing. A live connector is not a live tunnel — which is
...
origin willing to serve the leaver on the canonical hostname, the exact path this build defends.
The lists had already drifted: Access allowed six, the origin one.
...
200 at once. Then Terraform planned the token delete *before* the policy update that
...
for twenty minutes and **the user found it, not me.** `.env.example` had no Turnstile
entry, so I invented `TURNSTILE_SITEKEY` while the code read `TURNSTILE_SITE_KEY`, and a
`?? ''` default meant the page rendered fine and failed only at submission. A fallback on a
*required* value turns a startup failure into a runtime one, and an incomplete `.env.example` is
a bug waiting to be guessed at. The sitekey is now required config.
...
and far more without the tail — the symptom names no layer.
...
pointed at; there is a lock now. Its log then showed a third fault: my hostname pattern matched
`api.trycloudflare.com`, which cloudflared prints when tunnel *creation* fails, so a surface was
briefly published pointing at Cloudflare's own API — a URL nothing throws on, and no probe calls
dead, because it answers.
...
console's own audience so `aud` cannot be what refuses it. The check is real; my account of where to
watch it was wrong.
...
§7 (SSH and WARP through Access, 7 points) and R6's firewall half need root on this box.
...
Cloudflare-issued identifiers changing underneath — the part that makes a rebuild hard.
